							...
							<tbody>
								<tr>
									...
									<td><fmt:formatDate value="${vo.regdate }"
											pattern="yyyy년MM월dd일  a hh:mm" /></td>
							...
							<ul class="pagination pagination-sm">
								...
								<c:forEach var="num" begin="${pageVO.startPage }"
									...
									<li class="${pageVO.pageNum == num ? 'active' : ''}"><a
										href="freeList?pageNum=${num}">${num }</a></li>
								</c:forEach>
...
<script>
	...
	//3. pageForm을 a태그 -> 폼전송 방식으로 변경
	//4. Criteria에 searchType, searchName추가
	//5. 게시글쿼리(getList), 전체게시글수(getTotal) 쿼리를 동적쿼리로 변경
	//6. 검색 후에 검색의 정보를 select박스에 키워드 처리, input태그에 키워드처리
	//7. 
	//검색처리(버튼 클릭 시 폼전송)
